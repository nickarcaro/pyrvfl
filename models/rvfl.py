...
if __name__ == "__main__":
    ...
    def train_grouped_rvfl(rvfl, n_clusters, data, label, n_class):
        # Aplicar K-means al conjunto de datos para encontrar n_clusters grupos
        kmeans = KMeans(n_clusters=n_clusters, random_state=0).fit(data)

        # Entrenar un RVFL para cada grupo
        rvfl_per_cluster = []
        for i in range(n_clusters):
            idx = np.where(kmeans.labels_ == i)
            cluster_data = data[idx]
            cluster_label = label[idx]
            # Entrenar RVFL para este grupo
            local_rvfl = RVFL(
                n_nodes=rvfl.n_nodes,
                lam=rvfl.lam,
                w_random_vec_range=rvfl.w_random_range,
                b_random_vec_range=rvfl.b_random_range,
                activation="relu",
                same_feature=False,
                task_type="classification",
            )
            local_rvfl.train(cluster_data, cluster_label, n_class)
            rvfl_per_cluster.append(local_rvfl)

        return rvfl_per_cluster, kmeans

    # En tu código principal...

    n_clusters = 3  # Número de grupos que quieres
    ...
    # Entrenar un RVFL para cada grupo y guardar el modelo k-means
    rvfl_per_cluster, kmeans = train_grouped_rvfl(
        rvfl, n_clusters, train[0], train[1], num_class
    )

    # Para predecir, usa el modelo k-means para asignar cada punto a un grupo, luego usa el RVFL correspondiente
    labels_val = kmeans.predict(val[0])

    acc = 0
    for i in range(n_clusters):
        idx = np.where(labels_val == i)
        cluster_data_val = val[0][idx]
        cluster_label_val = val[1][idx]
        local_acc = rvfl_per_cluster[i].eval(cluster_data_val, cluster_label_val)
        acc += local_acc * len(cluster_label_val)
